...
async def get_r2_session(
    ...
    """
    Creates an authenticated aioboto3 session configured for Cloudflare R2.
    Caches the session until expiration to avoid repeated authentication.

    Returns:
        An authenticated aioboto3 Session configured for R2
    """
    ...
    async with GLOBAL_SESSIONS.get_lock():
        ...
        # Create a new session with R2-specific configuration
        ...
        # R2 sessions don't expire in the same way as AWS sessions
